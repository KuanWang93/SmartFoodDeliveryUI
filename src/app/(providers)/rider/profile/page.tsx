  ...
  useEffect(() => {
    async function fetchProfile() {
      try {
        ...
        if (!data || !data.rider) throw new Error('Missing profile data')
        setForm({
          username: data.username || '',
          email: data.email || '',
          phone: data.rider.phone || '',
          gender: data.rider.gender || '',
          avatar: data.rider.avatar || '',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-white to-pink-100 flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-2xl bg-white rounded-3xl shadow-xl p-10">
        <h2 className="text-3xl font-extrabold text-indigo-700 mb-2 text-center tracking-tight">Rider Profile</h2>
        <p className="text-center text-gray-500 mb-8">View and update your rider account</p>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="space-y-5">
            <div>
              ...
              <input type="text" name="username" value={form.username} disabled className="w-full p-3 border rounded-xl bg-gray-100 cursor-not-allowed" />
            </div>
            <div>
              <label className="block font-semibold mb-1">Email <span className="text-red-500">*</span></label>
              <input type="email" name="email" value={form.email} disabled className="w-full p-3 border rounded-xl bg-gray-100 cursor-not-allowed" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={form.createTime} disabled className="w-full p-3 border rounded-xl bg-gray-100 cursor-not-allowed" />
            </div>
            <div>
              <label className="block font-semibold mb-1">Updated At</label>
              <input type="text" value={form.updateTime} disabled className="w-full p-3 border rounded-xl bg-gray-100 cursor-not-allowed" />
            </div>
            <button
              type="submit"
